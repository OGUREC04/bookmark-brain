/* ChatRow — telegram-style row for the feed
   avatar + name/time | preview/badge.  dense, scannable, single tap. */
...
function Avatar({ kind='letter', tone='sage', letter='B', glyph, icon, style={} }) {
  const base = {
  ...
  if (kind === 'task') {
    return (
      <div style={{
        ...
        {React.cloneElement(Icons.task, { size: 22, sw: 1.6 })}
      </div>
    );
  }
  if (kind === 'archive') {
    return (
      <div style={{
        ...base,
        background: 'rgba(234,227,207,0.9)',
        border: '1px dashed var(--border-strong)',
        color: 'var(--fg-3)',
      }}>
        {React.cloneElement(Icons.archive, { size: 20, sw: 1.4 })}
      </div>
    );
  }
  if (kind === 'icon') {
...
function ChatRow({
  avatar,            // { kind, tone, letter, glyph, icon }
  name,
  time,
  timeNow=false,
  pinned=false,
  preview,           // node — can contain <b> highlights
  src,               // small mono prefix like "x.com"
  unread=false,
  badge=null,        // number for unread badge
  star=false,
  pulsing=false,
  done=false,
  muted=false,       // for archive
  onClick,
  isLast=false,
}) {
  return (
    ...
    onMouseEnter={e=>{e.currentTarget.style.background='rgba(255,252,246,0.35)';}}
    onMouseLeave={e=>{e.currentTarget.style.background='transparent';}}
    >
      {unread && (
        <span style={{
          position: 'absolute', left: 6, top: '50%', transform: 'translateY(-50%)',
          width: 3, height: 22, borderRadius: 2, background: 'var(--brand-primary)',
        }}/>
      )}
      ...
      <div style={{
        ...
        {!isLast && (
          <span style={{
            position: 'absolute', left: 0, right: -4, bottom: 0,
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {pinned && (
            <span style={{ color: 'var(--fg-3)', transform: 'rotate(40deg)', flexShrink: 0, display: 'flex' }}>
              {React.cloneElement(Icons.pin, { size: 13, sw: 1.6 })}
            </span>
          )}
          <span style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{
            ...
            fontSize: 13.5, color: muted ? 'var(--fg-4)' : 'var(--fg-2)',
            ...
            {done && (
              <span style={{ display:'inline-flex', verticalAlign:'-1.5px', marginRight:6, color:'var(--brand-primary)' }}>
                {React.cloneElement(Icons.check, { size: 11, sw: 2.5 })}
              </span>
            )}
            {src && (
              <span style={{
                ...
                letterSpacing: '.04em', marginRight: 6,
              }}>{src}</span>
            )}
            {preview}
          </span>
          {badge != null && (
            <span style={{
              flexShrink: 0, minWidth: 20, height: 20, padding: '0 7px',
              borderRadius: 999,
          ...
          {star && (
            <Glyph ch="★" size={15} color="var(--brand-primary)"/>
          )}
...
Object.assign(window, { ChatRow, DaySeparator, Avatar, avatarGradients });
