...
# Компенсация: нумерованный диктат ИЛИ повтор-императив → TODO.
# Подтверждение «Сделать список?» спросит дальше — даже ложный
# позитив гасится отказом.

# «1 ... 2 ... 3 ...» — нумерованный диктат без точек после цифр.
# Порог ≥3 (а не ≥2) — иначе «у меня 2 идеи 3 варианта» уходит в
# TODO. 2-цифровое перечисление слишком частое в обычной речи.
# См. code review H2.
_NUMBERED_LIST_RE = re.compile(r"(?:^|\s)[1-9]\b")
_NUMBERED_LIST_MIN = 3
...
def detect_intent(text: str, duration: float | None = None) -> IntentResult:
    ...
    # ≥3 цифр-маркеров ИЛИ ≥2 «нужно/надо <глагол>» → диктовка
    # списка. Порог 3 для цифр снижает false-positive («у меня 2
    # идеи 3 варианта» = 2 цифры → НЕ TODO). Императивы оставляем
    # ≥2 — пара повторов «нужно X нужно Y» это уже сильный сигнал.
    if (
        len(_NUMBERED_LIST_RE.findall(normalized)) >= _NUMBERED_LIST_MIN
