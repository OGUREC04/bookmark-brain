...
class ParseStatus(str, Enum):
    ...
    # (это именно «нет часа», а не «нет времени вообще» — может быть и часть суток
    # «утром/вечером», но та маппится в _preprocess_short_time и возвращает OK).
...
def parse(
    ...
    # наследует текущий час → NEEDS_HOUR не срабатывает. «сегодня» не трогаем.
    base = now_in_user_tz
    ...
    # это дата без времени → NEEDS_TIME (has_time_in_text/has_interval посчитаны выше)
